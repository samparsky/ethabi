...
mod tests {
    ...
    fn test_encoding_function_input_as_array() {
        ...
        let encoded_from_vec =
            functions::set_validators::encode_input(vec![first.clone(), second.clone()]);
        let encoded_from_vec_iter = functions::set_validators::encode_input(
    ...
    fn test_decoding_function_output() {
        ...
        use eip20;
        let output =
            hex!("000000000000000000000000000000000000000000000000000000000036455B").to_vec();
    ...
    fn test_encoding_constructor_as_array() {
        ...
        let encoded_from_vec_iter = constructor(
            code.clone(),
            vec![first.clone(), second.clone()].into_iter(),
        );
        let encoded_from_vec_wrapped =
            constructor(code.clone(), vec![Wrapper(first), Wrapper(second)]);
    ...
    fn test_encoding_function_input_as_fixed_array() {
        ...
        let encoded_from_array =
            functions::add_two_validators::encode_input([first.clone(), second.clone()]);
        ...
        assert_eq!(
            expected_array,
            encoded_from_array_wrapped.to_hex::<String>()
        );
